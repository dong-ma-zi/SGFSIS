"""
func: with sematic and contour guidance
"""
...
def get_parser():
    ...
    parser.add_argument('--config', type=str, default='./config/ext_joint_training/monusac.yaml',
                        help='config file')
...
def train_one_epoch(train_loader, model, optimizer, epoch, num_class):
    ...
    for i, (img_input, cls_target, fore_target, _,
        ...
        loss = loss + cls_meta_loss + sem_meta_loss + con_meta_loss # cen_meta_loss # + con_meta_loss

        # meta loss
        accu_loss["sup"]['cls_metabce'] += cls_meta_loss.cpu().item()
        accu_loss["sup"]['sem_metabce'] += sem_meta_loss.cpu().item()
        accu_loss["sup"]['cen_metabce'] += 0 # cen_meta_loss.cpu().item()
...
def evaluate_one_epoch_patch_level(model,
    ...
    for i, (img_input, cls_target, fore_target, inst_target, _, _, _) in enumerate(test_loader):
        ...
        with torch.no_grad():
            pred_dict_, sem_sim_map, _, con_sim_map = \
    ...
    for idx, result in enumerate(results):
        ...
        sem_sim_map, con_sim_map = result
        ...
        if is_draw and num_run == 1:
            ...
            norm_img = cv2.normalize(con_sim_map, None, 32, 224, cv2.NORM_MINMAX)
            norm_img = np.asarray(norm_img, dtype=np.uint8)
            con_sim_heat_img = cv2.applyColorMap(norm_img, cv2.COLORMAP_JET)
