...
from models.SGFSL_GCMvar1 import SGFSL
...
def model_init():
    ...
    load_weight_dict = {k: v for k, v in model_weight.items()
                        if model.state_dict()[k].numel() == v.numel() and 'decoder.tp' not in k}
...
def main_worker(argss):
    ...
    for run, (tsk_loader, tsk_loader_regis) in enumerate(zip(tsk_loader_list, tsk_loader_regis_list), start=5):
