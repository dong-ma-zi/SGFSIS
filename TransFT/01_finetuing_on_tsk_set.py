...
def main(args):
    ...
    if not args.full_sup and not os.path.exists(f'../rand_split_file_{args.tsk_dataset}/labeled_shot_{args.shot}'):
        os.makedirs(f'../rand_split_file_{args.tsk_dataset}/labeled_shot_{args.shot}', exist_ok=True)
        train_split_shot(base_dataset=base_dataset, seed_list=args.split_seed,
                         shot=args.shot, dataset=args.tsk_dataset, num_type=args.num_type)
    ...
    for run in range(1, num_run + 1):
        ...
        if not args.full_sup:
            with open(f'../rand_split_file_{args.tsk_dataset}/labeled_shot_{args.shot}/split_{run}.pkl', 'rb') as f:
                split = pickle.load(f)
        ...
        # get model weight pretrained on ext dataset
        weight_path = glob.glob(f'./exp/ext_{args.ext_dataset}/run_1/*.pth')
        assert len(weight_path) == 1
        weight_path = weight_path[0]

        # assign model
        model = create_model(args.device, weight_path=weight_path, num_type=args.num_type)
        ...
        for epoch in range(1, args.epochs + 1):
            ...
            if epoch % args.epochs == 0 or epoch % 50 == 0:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--visible_device', type=str, default="3", help="gpu can be visual")

    parser.add_argument('--ext_dataset', type=str, default='pannuke', help="dataset: lizard | pannuke")
    parser.add_argument('--tsk_dataset', type=str, default='monusac', help="dataset: lizard | pannuke")
    ...
    parser.add_argument('--full_sup', type=bool, default=False, help="is fully supervise the model training")
    ...
    parser.add_argument('--milestones', type=list, default=[50], help="学习率更新的epoch")
