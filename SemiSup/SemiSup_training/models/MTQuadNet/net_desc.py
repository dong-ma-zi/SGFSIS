...
# from models.hovernet.utils import dice_loss, mse_loss, msge_loss, xentropy_loss, crop_op, crop_to_shape
...
#     def __init__(self, pretrained_backbone=None, num_types=0):
#         super(EntireModel, self).__init__()
#         self.sup_loss_np = nn.CrossEntropyLoss()
#         self.teacher = QuadNet(pretrained_backbone=pretrained_backbone, num_types=num_types)
#         self.student = QuadNet(pretrained_backbone=pretrained_backbone, num_types=num_types)
#         self.epoch_semi = 6
#         self.threshold = 0.5
#
#
...
#
#
#     def warm_up_forward(self, x_l):
#         """
#         Using the labeled data to warm up teacher model
#         :param x_l:
#         :return:
#         """
#
#         pred_dict_l = self.student(x_l)
#         pred_dict_l = OrderedDict(
#             [[k, v.permute(0, 2, 3, 1).contiguous()] for k, v in pred_dict_l.items()]
#         )
#         pred_dict_l["np"] = F.softmax(pred_dict_l["np"], dim=-1)
#         pred_dict_l["tp"] = F.softmax(pred_dict_l["tp"], dim=-1)
#
#         return pred_dict_l
#
#
...
#
#         if warm_up:
#             return self.warm_up_forward(x_l=x_l, true_dict_l=true_dict_l)
#
#         # supervised output
#         pred_dict_l = self.student(x_l)
#         pred_dict_l = OrderedDict(
...
#
#         # unsupervised output
#         pred_dict_ul = self.student(x_ul)
#         pred_dict_ul = OrderedDict(
#             [[k, v.permute(0, 2, 3, 1).contiguous()] for k, v in pred_dict_ul.items()]
#         )
#         pred_dict_ul["np"] = F.softmax(pred_dict_ul["np"], dim=-1)
#         pred_dict_ul["tp"] = F.softmax(pred_dict_ul["tp"], dim=-1)
#
#         output = {'sup_pred': pred_dict_l, 'unsup_pred': pred_dict_ul}
#
#         return output
