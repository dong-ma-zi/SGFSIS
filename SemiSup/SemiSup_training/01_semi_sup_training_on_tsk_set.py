...
parser.add_argument('--dataset', type=str, default='consep', help='monusac lizard...')
...
parser.add_argument('--l_batch_size', type=int, default=2)
parser.add_argument('--ul_batch_size', type=int, default=2)
parser.add_argument('--num_workers', type=int, default=0)
parser.add_argument('--lr', type=float, default=5e-5) # 5e-5 for consep 1e-5 for pannuke
parser.add_argument('--weight_decay', type=float, default=5e-4)
parser.add_argument('--warm_up_epoch', type=int, default=1, help='warm up epoch for mean teacher')
parser.add_argument('--epoch', type=int, default=50, help='training epoch for mean teacher')
parser.add_argument('--save_per_epoch', type=int, default=50, help='save model per epoch')
...
class Trainer:
    def __init__(self, teacher, student, num_types, run,
        ...
        self.optimizer = torch.optim.Adam(self.student.module.parameters(), lr=args.lr, weight_decay=args.weight_decay)
    ...
    def update_teachers(self, alpha=0.995):
        with torch.no_grad():
            for param_teacher, param_student in zip(self.teacher.module.parameters(), self.student.module.parameters()):
    ...
    def _train_epoch(self, num_type):
        ...
        for _, (data_dict_l, data_dict_ul) in enumerate(zip(self.supervised_loader, self.unsupervised_loader)):
            if self.mode == 'semi':
                ...
                loss_total = loss_sup + 10 * loss_unsup
                accu_loss["total"] += loss_total.cpu().item()

                self.optimizer.zero_grad()
                loss_total.backward()
                self.optimizer.step()
                self.update_teachers(alpha=0.995)
        return accu_loss
